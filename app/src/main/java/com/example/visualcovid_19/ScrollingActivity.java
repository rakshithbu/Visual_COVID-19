...
public class ScrollingActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    private ArrayList<String> starredCountries;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // db.addStarredCounties(String.valueOf("USA"));
        if (isOnline()) {
...
//        googleMap.moveCamera(CameraUpdateFactory.newLatLng(sydney));
    ...
    public void addCountryCards(){
        ...
        if(findViewById(R.id.cardsContainer) != null) {
            ...
            JSONArray notStarred = new JSONArray();
            ...
            for (int i = 0; i < covidData.length(); i++){
                try {
                    ...
                    if(starredCountries.contains(countryName)){
                        starred.put(covidData.get(i));

                    }else{
                        notStarred.put(covidData.get(i));
                    }
            ...
            if(starred.length()>0) {
                for (int i = 0; i < starred.length(); i++) {
                    try {
                        JSONObject countryData = starred.getJSONObject(i);
                        String countryName = countryData.getString("country");
                        String formattedCasesCount = NumberFormat.getNumberInstance(Locale.US).format(countryData.getInt("cases"));
                        CountryCardFragment cardFragment = CountryCardFragment.newInstance(countryName, formattedCasesCount, true);
                ...
                for (int i = 0; i < notStarred.length(); i++){
                    try {
                        JSONObject countryData = notStarred.getJSONObject(i);
                        String countryName = countryData.getString("country");
                        String formattedCasesCount = NumberFormat.getNumberInstance(Locale.US).format(countryData.getInt("cases"));
                        CountryCardFragment cardFragment = CountryCardFragment.newInstance(countryName, formattedCasesCount,false);
                        fragmentTransaction.add(R.id.cardsContainer, cardFragment, null);
